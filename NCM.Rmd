...
forceUpdate <- FALSE
if(file.exists("supdat.RData") && !forceUpdate) {
  load("supdat.RData")
} else {
  supdat <- read.csv("Season Clients Master_20170222-122327.csv")
  #supdat <- read_csv("Season Clients Master_20170222-122327.csv", trim_ws = TRUE)
  save(supdat, file="supdat.RData")
}
...
# use X2017..Long.Rain_Maize.acres instead?
...
Cdata$maize.adoption <- ifelse(Cdata$X2017..Long.Rain_Maize.acres==0,0,1)
Tdata$maize.adoption <- ifelse(Tdata$X2017..Long.Rain_Maize.acres==0,0,1)
...
* H8	Average 1AF Client Density (% total households joining 1AF)
...
###Poor statistical power was preventable

ML note: explain parametric and non-parametric in a glossary.
...
Added a couple regressions, cleaned up language a bit.
...
Looking at bean adoption as a worked example, if we run both the normal (invalid) power calculation, and the non-normal (valid) power calculation, the simulations suggests a 25% increase in bean adoption would only yield a power of `r round(pt$power,3)` for normally distributed data and `r round(tp,3)` for non-normal data. This power is clearly insufficient for a robust analysis that is able to distinguish between there being no effect, and being unable to detect an effect. 

Going forward, I have provided a short R script for the easy calculation of statistical powers under normal and non-normal regimes ([here](  https://drive.google.com/open?id=0B3_WyX1D8qwMYVI1MnlBYlNCQ1k  "Link")).
...
print(multiplot(h1,h2,h3,h4,h5,h6,  cols=2))
...
ML formatting note: use knitr::kable to format the wilcox test more nicely or maybe just so the p-value. I try to avoid just having raw R output.
...
```{r, out1, results='asis'}
#stargazer(acre.rob, type = "html")
```
Here we have a table of terms and constants with confidence intervals in brackets. A positive term is interpreted such that an increase in 1 unit of x (eg. 1 more season enrolled) increases the y outcome by the noted amount. Additionally, the presence of 2 or more asterix indicated statistical significance (the coefficient is significantly different from zero). For "TCTreated", the coefficient represents the difference between control and treatment group. 
...
```{r, results='asis'}
stargazer(acre.rob, acre.bean, acre.stove, acre.solar, type = "html",
          omit="District",
          covariate.labels = c("Treatment", "OAF Seasons"),
          dep.var.labels = c("Maize acres", "Beans", "Stove", "Solar"))
```
...
ML note: I combined the tables into one regression as our X vars were the same.
...
stargazer(acre.maizchoice, type = "html",
          omit="District")
```
...
nclients <-lmrob(formula = transNclients ~ TotalEnrolledSeasons + TC + District, data=msite)
stargazer(nclients, type = "text")
...
Once again, non-normal residuals means we need to transform our Y-variable. In this case a square root transform yields an R-squared of `r, round(x1,3)` against a normal distribution. Examination of the coefficients reveals no significant relationships were detected. 
...
**ML note**: can you add a summary table that goes hypothesis by hypothesis and provides both a coefficient, indication of significance, and text interpretation? Or alternatively, it shows hypothesis This would be the quick glance summary of results by hypothesis and you can link to it from the key takeaways.
